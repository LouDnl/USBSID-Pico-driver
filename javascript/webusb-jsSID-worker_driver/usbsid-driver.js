 ...
 *
 * usbsid-driver.js (Javascript driver)
...
/* https://github.com/pubkey/rxdb/issues/5990#issuecomment-3406504579 */
function deepCloneNoProto(obj) {
  if (obj === null || typeof obj !== "object") return obj;
  if (Array.isArray(obj)) return obj.map(deepCloneNoProto);
  const clone = Object.create(null);
  for (const key of Object.keys(obj)) {
    clone[key] = deepCloneNoProto(obj[key]);
  }
  return clone;
}
...
class USBSID {
  #_worker; #_channel;
//  #_URL = globalThis.location.origin + '/workertest';
  ...
  _init_worker()
  {
    ...
    if(typeof(this.#_worker) == "undefined") {
      this.#_worker = new Worker(this.#_URL + "/usbsid-worker.js");
      this.#_worker.onmessage = function(event) {
        this._debug_driver("W: ", event.data);
      }
      this._debug_driver("worker started");
    } else {
      this._debug_driver("worker already started");
    }
  }
  _init_channel() {
    if (this.#_channel == undefined) {
      ...
      this.#_channel.onmessage = function (m) {
        this._debug_driver('Received from worker ', m/* .data */);
        if(m.data.connected == true) {
          this.isopen = m.data.connected;
        // this.isopen = m.data.connected;
        }
      }.bind(this);
      this._debug_driver("channel opened");
    } else {
      this._debug_driver("channel already open");
    }
  }

  _debug_driver(...message)
  {
    if (this._debug) {
      console.info('%cDRIVER:', 'background: #00aeffff; color: #000000ff; font-weight: bold;',  ...message);
    }
  }
  ...
  async #goWorker(data = null)
  {
    // this._debug_driver("goWorker:", data);
    await this.#_channel.postMessage(data);
  }
  ...
  async init(dev) {
    try {
      ...
      //     });
      //   }
        await self.#goWorker({init: true/* , device: deepCloneNoProto(self.device) */});
      }
    } catch (err) {
      console.error(err);
    }
  ...
  async write(buffer) {
    this.#goWorker({write: true, buffer: buffer});
  ...
  async close() {
    this.isClosing = true;
    await this.#goWorker({deinit: true});
    // try {
    ...
    if (typeof(this.#_worker) != "undefined") {
      ...
      this.#_worker = undefined;
      this._debug_driver("worker stopped");
    } else {
      this._debug_driver("worker not started");
    }
  }
